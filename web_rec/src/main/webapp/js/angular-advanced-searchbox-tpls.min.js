                ...
                controller: [
                    ...
                    function ($scope, $attrs, $element, $timeout, $filter) {
                        if($scope.selectedNode!=undefined){
                            $scope.searchParams =$scope.selectedNode;
                        }else{
                            $scope.selectedNode=[];
                        }
                        $scope.searchQuery = '';
                        ...
                        $scope.$watch('searchQuery', function (e) {
                            updateModel();
                        });
                        ...
                        $scope.addSearchParam = function (searchParam, value, enterEditModel) {
                            if (enterEditModel === undefined)
                                enterEditModel = true;
                            if($scope.single && $scope.searchParams.length>0){
                                $scope.searchParams[0].key="id";
                                $scope.searchParams[0].name=searchParam.name;
                                $scope.searchParams[0].value=searchParam.id;
                                $scope.searchParams[0].editMode=enterEditModel;
                                return false;
                            }
                            var flag=false;//是否已经存在
                            angular.forEach($scope.searchParams,function(item){
                                if(item.value==searchParam.id)flag=true;
                            })
                            if(!flag){
                            ...
                            searchThrottleTimer = $timeout(function () {
                                ...
                                angular.forEach($scope.searchParams, function (param) {
                                    if (param.value !== undefined && param.value>0){
                                        if($scope.model[param.key]==undefined){
                                            $scope.model[param.key] = param.value;
                                        }else{
                                            $scope.model[param.key] = $scope.model[param.key]+"~"+param.value;
                                        }
                                    }
                                });
                            }, 500);
